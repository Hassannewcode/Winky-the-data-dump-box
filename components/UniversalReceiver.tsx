...
import { Radio, MessageSquare, Activity, Globe, Code2, ArrowRight, Zap, Ghost } from 'lucide-react';
...
export const UniversalReceiver: React.FC<UniversalReceiverProps> = ({ 
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === 'WINKY_SIGNAL_INGEST') {
        onLog("Shadow Protocol Ingest", "SUCCESS", "Headless Background Packet");
        onDataReceived(PacketSource.HEADLESS, event.data.data, event.data.label || 'Shadow Protocol');
        return;
      }
      ...
      >
        <div className="flex flex-col items-center justify-center text-center space-y-6 py-4 relative z-10">
          ...
          <div className="flex flex-wrap justify-center gap-2 w-full max-w-xs">
             <div className="flex items-center gap-1.5 text-[9px] font-bold text-winky-text-soft bg-winky-bg px-3 py-2 rounded-xl border border-winky-border hover:border-winky-pink transition-colors cursor-default" title="Shadow Ingest Active"><Ghost className="w-3 h-3 text-purple-500" /><span>SHADOW</span></div>
