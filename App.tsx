...
const App: React.FC = () => {
  // Check for payload immediately to prevent UI flash
  const checkInitialPayload = () => {
    const params = new URLSearchParams(window.location.search);
    return params.has('payload') || params.has('data') || params.has('q');
  };

  // --- STEALTH LOGIC STATE ---
  const [isStealthMode, setIsStealthMode] = useState(checkInitialPayload());
      ...
      const successTimer = setTimeout(() => {
        ...
        const exitTimer = setTimeout(() => {
          ...
          if (redirectUri && redirectUri !== window.location.href) {
            ...
            try {
              // Try closing the tab if it's a standalone popup
              window.close();
              // If window.close() fails (browser restriction), clear state to show dashboard
              setTimeout(() => {
        ...
        }, 300); // Reduced delay for maximum speed

        return () => clearTimeout(exitTimer);
      }, 400); // Reduced delay for visual confirmation
  ...
  return (
    <div className="h-screen w-full bg-winky-bg text-winky-text font-sans flex flex-col overflow-hidden transition-colors duration-300">
      {/* High-Speed Stealth Overlay: Only visible during headless ingestion */}
      {isStealthMode && <StealthPulse status={stealthStatus} onComplete={() => setIsStealthMode(false)} />}

      {/* Main UI: Hidden entirely during stealth mode to ensure a clean loading experience */}
      {!isStealthMode && (
        <>
          <nav className="shrink-0 bg-winky-card/80 backdrop-blur-md border-b border-winky-border">
            <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                 ...
                 <button onClick={() => setShowUrlInspector(true)} className="p-2 text-winky-text-soft hover:text-winky-blue transition-colors rounded-full hover:bg-winky-bg" title="URL Scanner">
                   <Link2 className="w-5 h-5" />
                 </button>

                 <button onClick={() => setShowConfig(true)} className="p-2 text-winky-text-soft hover:text-winky-blue transition-colors rounded-full hover:bg-winky-bg" title="Settings">
